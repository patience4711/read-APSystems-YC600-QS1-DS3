...
if( Inv_Prop[which].invType == 2 ) 
      { // is this a DS3, 
        if( diagNose != 0 ) ws.textAll( "decoding a DS3 inverter");
        // ACV offset 34
        Inv_Data[which].acv = extractValue(68, 4, 1, 0, s_d) / 3.8 ;
        if( diagNose != 0 ) ws.textAll( "extracted ACV = " + String(Inv_Data[which].acv) );
        
        // FREQ offset 36
        Inv_Data[which].freq = extractValue(72, 4, 1, 0, s_d)/100;
        if( diagNose != 0 ) ws.textAll( "extracted FREQ = " + String(Inv_Data[which].freq) );
        
        // HEATH offset 48        
        Inv_Data[which].heath = extractValue(96, 4, 1, 0, s_d)*0.0198 - 23.84; //18
        if( diagNose != 0 ) ws.textAll( "extracted HEATH = " + String(Inv_Data[which].heath) );
