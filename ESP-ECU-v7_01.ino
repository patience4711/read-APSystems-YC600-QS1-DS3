...
int errorCode=10;
int recovered = 0;
  time_t t_new[YC600_MAX_NUMBER_OF_INVERTERS] = {0};
  float energy_new[YC600_MAX_NUMBER_OF_INVERTERS][4] = {0};
...
//  // ****************** mqtt init *********************
  ...
  if ( Mqtt_Enabled ) 
  {
       DebugPrintln("setup: mqtt configure");
   ...
   if(now() > switchonTime && now() < switchoffTime) 
    {
         ...
         if(dayTime) 
         {
            ...
            actionFlag = 301; // triggers the null message at midnight
         }
    }


// ******************************************************************
//              polling every pollres seconds
// ******************************************************************

  unsigned long nu = millis();  // de tijd dat het programma al loopt
   if (nu - laatsteMeting >= 1000UL * pollRes) 
   {
        laatsteMeting += 1000UL * pollRes ; // wordt iedere keer (meetRes * miliseconden) groter;
         //if(now() > switchonTime && now() < switchoffTime )
...
//  if(Mgtt_Enabled) { if (!MQTT_Client.loop()) mqttConnect; }
  ...
  if(Mqtt_Enabled) MQTT_Client.loop();
...
EEPROM.commit();
  ...
  // ******************  reset the nework data and reboot in AP *************************
    ...
    if (actionFlag == 47) { //triggered by the webpage zbtest
        actionFlag = 0; //reset the actionflag
        polling(iKeuze); 
    }
}
