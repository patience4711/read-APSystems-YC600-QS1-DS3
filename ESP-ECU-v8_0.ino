#define VERSION  "ESP-ECU-v8_0_BETA6"
...
int recovered = 0;
  ...
  time_t t_saved[YC600_MAX_NUMBER_OF_INVERTERS] = {0};
...
EEPROM.commit();
  ...
  // ******************  reset the nework data and reboot in AP *************************
    ...
    if (actionFlag == 122) {
      actionFlag = 0; //reset the actionflag
      testDecode(1); // test a fake pollinganswer for yc
    }
    if (actionFlag == 123) {
      actionFlag = 0; //reset the actionflag
      testDecode(2); // test a fake pollinganswer for qs
    }    
#endif
