/* recent changes are sLen in sendZigbee and ws.all(message) in sendZigbee
 *  edit processIncomingByte no need for anymore, this is only used by readZigbee
 *  now xperinent with non global messageToDecode
 *  trasferred waitSerialavailable to readzigbee
 *  ReadZigbee returns a char containing the incoming message, global inMessage redundant.
 */
#define VERSION  "ESP-ECU_v9_11_exp"
   ...
   if(now() > switchonTime && now() < switchoffTime) 
    {
         ...
         if(dayTime) 
         {
            ...
            Update_Log("system", term);
            ws.textAll(term);
            // clean memory makes the zigbee crash
...
//   }
  ...
  //check mqtt is connected when it should (Mqtt_Format !0)
  ...
  if(hour() == 0 && timeRetrieved && actionFlag == 301)
  {
      if(second() > 0 ) 
      {
        ...
        diagNose = 0; // incase this was forgotten
       }
  }
