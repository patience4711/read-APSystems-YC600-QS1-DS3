...
//can we pair when the radio is up for normal operation? No
   
   String term = "start pairing inverter sn " + String(Inv_Prop[iKeuze].invSerial);
   Update_Log("pairing", term);
   //suppress the consoleoutput while starting the coordinator
   diagNose = false;
    if( !coordinator(false) ) { // start coordinator for pairing
...
// we call this function when coordinator is up for pairing
    ...
    for (int y = 2; y < 6; y++) 
    {
      ...
      // add CRC at the end done in sendZigbee
      ...
      // so we analyze the anwer then, otherwise read and ignore
      if(y == 3 || y == 4) {
        ...
        waitSerialAvailable();
        empty_serial(); // read these bytes but throw them away
      }
   }
...
bool decodePairMessage(int which)
{
    ...
    char * messageToDecode = readZigbee(s_d);
    ...
    if(diagNose) ws.textAll("decoding answer " + String(messageToDecode));     
