...
inverterdata Inv_Data[MAX_NUMBER_OF_INVERTERS];
...
//   if (nu - laatsteMeting >= 1000UL * (300 / inverterCount)) 
   ...
   {
     if(diagNose) ws.textAll("300 secs passed" + String(millis() ) ); //
...
// ******************************************************************
   nu = millis() + 1000UL * 120 ; // 720 sec = 12 minutes later
