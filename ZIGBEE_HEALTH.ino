...
void healthCheck() {
    ...
    if(Mqtt_Format != 0 && Mqtt_Format != 5 && domIdx != 0) {
       if (mqttConnect() ) {
            char Mqtt_send[26];
        ...
        sprintf( toMQTT, "{\"idx\":%d,\"svalue\":\"%ld;%ld\"}", domIdx, ESP.getFreeHeap(), ESP.getFreeContStack()); 
        //must be like {"idx":901,"nvalue":0,"svalue":"22968;1904"}
   ...
   if(!timeRetrieved) getTijd();
    ...
    if(errorCode >= 100) errorCode -= 100;
        ...
        //ZigbeePing();
        // if the loopback fails, we don't start the coordinator
        if( !ZigbeeLoopBack() ) {
          ws.textAll(F("no zigbee hardware"));
          Update_Log( 2, "no hardware");
          zigbeeUp = 0;
          return;
        }
...
// we evaluate that
    ...
    empty_serial(); 
      ...
      // when oke we get this : FE0E670000 FFFF80971B01A3D8 0000 07090011
      // received : FE0E670000FFFF80971B01A3D600000709001F 
...
void ZBhardReset() 
    ...
    ws.textAll(F("ZBmodule hard reset"));
